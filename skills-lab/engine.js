...
export const required={
 ...
 face:pair('Keep the face clear in prone','أبقِ الوجه دون ضغط في الانبطاح'),
...
export class Simulation {
 ...
 if(action==='comfort'){
  ...
  if(s.scenario==='prone'&&s.head<65){s.flash='head';return this.event('safety','face',msg.prone,'R20');}
 ...
 omissions(){let s=this.s;const done={...s,arm:s.farArm>=75&&s.nearArm>=70,leg:s.leg>=70,movement:s.progress>=1,supports:this.allSupports(),face:s.scenario!=='prone'||s.head>=65,lower:s.height<=40};return Object.keys(required).filter(k=>!done[k]);}
 hint(){const s=this.s;if(s.progress>=1){if(s.scenario==='prone'&&s.head<65)return 'head';if(!this.allSupports())return s.held?'support_'+this.supportKeys().find(k=>!s.supports[k]):'pillows';if(!s.comfort)return 'patient';if(s.height>40)return 'bed';if(!s.bell)return 'bell';if(!s.post)return 'sink';return 'finish';}if(!s.talk)return 'patient';if(!s.assess)return 'patient';if(!s.hygiene)return 'sink';if(!s.privacy)return 'curtain';if(!s.brakes||!s.rail||s.height<55||s.height>85)return 'bed';if(!s.helper||!s.sheet)return 'sheet';if(s.farArm<75)return 'farArm';if(s.nearArm<70)return 'nearArm';if(s.leg<70)return 'leg';if(!this.isReadyStance())return 'stance';if(s.progress===.5&&!s.pauseChecked){if(s.scenario==='prone'&&s.nearArm<90)return 'nearArm';if(s.scenario==='prone'&&s.head<65)return 'head';return 'body';}return 'body';}
